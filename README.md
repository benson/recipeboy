...
Every recipe has a canonical `https://recipeboy.bensonperry.com/recipe/:id` URL, used by the address bar, **More → Copy recipe link**, and linked-recipe anchors. It serves the full app with the recipe's title, description, canonical URL, and Open Graph/Twitter metadata in the initial HTML. The latest active meal photo takes precedence over the imported source image; recipes without either have a text preview. Recipes without a description use their servings, time (including estimate markers), and ingredients. Each request reads the current recipe, so additions and edits need no frontend rebuild. Missing or deleted recipes return 404 without their old content.

The app opens the modal directly at that path, including after a refresh. Closing it returns to `/`; Back/Forward navigates between recipes and the box. Old `#recipe=:id` links upgrade to the canonical path when opened, and the previous Worker's `/share/:id` URLs permanently redirect there. Legacy fragments still cannot identify a recipe to crawlers until someone opens and copies the updated URL.

Hosting stays on GitHub Pages. Cloudflare proxies the existing `recipeboy → benson.github.io` CNAME; only `recipeboy.bensonperry.com/recipe/*` is routed to `recipeboy-api`. That Worker reads the published homepage shell from `/`, replaces only its metadata, and serves it at the recipe path with root-relative asset resolution. It never forwards visitor credentials when fetching the public shell. The route is managed in the Cloudflare dashboard because the deployment token cannot manage zone routes; leave `route`/`routes` unset in Wrangler to preserve dashboard configuration. Do not route the homepage through this Worker, which would make the shell fetch recursive.

Run `npm test` for crawler, navigation parsing, and recovery checks. The Playwright CLI's `run-code --filename scripts/check-recipe-sharing.js` checks production copy/open, metadata, photos, refresh, history, and legacy links without changing recipe data.
